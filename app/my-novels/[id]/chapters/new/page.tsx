...
import { ChevronLeft, Save, XCircle, Loader2, AlertCircle } from "lucide-react";
import Link from "next/link";
import { useAuth } from "@/lib/auth-context";
import apiClient from "@/lib/api-client";
import { NovelResponse } from "@/types/api";

export default function CreateMyChapterPage({
  params,
}: {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        console.log("Debug - user.id:", user.id.toString(), typeof user.id);
        ...
        if (!ownerId || parseInt(ownerId.toString()) !== user.id) {
    ...
    fetchData();
  }, [novelId, user, isAuthenticated, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient.createChapter(novelId, chapterData);

      router.push(`/my-novels/${novelId}/chapters`);
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center">
            <Link
              href={`/my-novels/${novelId}/chapters`}
